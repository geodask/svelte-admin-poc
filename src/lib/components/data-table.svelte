	...
	<Tabs.Content value="outline" class="relative flex flex-col gap-4 overflow-auto px-4 lg:px-6">
		<div class="overflow-hidden rounded-lg border">
			<DragDropProvider
				...
				onDragEnd={(e) => (data = move(data, e) as Record<string, unknown>[])}
			>
				<Table.Root>
...
{#snippet DraggableRow({ row, index }: { row: Row<Record<string, unknown>>; index: number })}
	{@const { ref, isDragging, handleRef } = useSortable({
		id: (row.original.id as string | number) ?? index,
		index: () => index
	})}
